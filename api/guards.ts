...
export async function findByEmail(email: string): Promise<GuardRow | null> {
  ...
  const [rows] = await db.execute<GuardRow[]>(
    "SELECT id, email, password_hash, role, full_name, gate, created_at, updated_at FROM guards WHERE email = ? LIMIT 1",
...
export async function createGuard(data: {
  ...
  await db.execute(
    "INSERT INTO guards (email, password_hash, role, full_name, gate) VALUES (?, ?, 'guard', ?, ?)",
    [email, passwordHash, data.fullName.trim(), gate]
  )
  const [rows] = await db.execute<{ id: number }[]>("SELECT LAST_INSERT_ID() AS id")
  const id = Array.isArray(rows) && rows[0] ? Number(rows[0].id) : 0
  return { id, email, role: "guard", fullName: data.fullName.trim(), gate }
}
...
export async function listGuards(): Promise<UserSession[]> {
  const db = getDb()
  const [rows] = await db.execute<{ id: number; email: string; full_name: string; gate: number | null }[]>(
    "SELECT id, email, full_name, gate FROM guards WHERE role = 'guard' ORDER BY full_name"
  )
  const list = Array.isArray(rows) ? rows : []
  return list.map((r) => ({ id: r.id, email: r.email, role: "guard" as const, fullName: r.full_name, gate: r.gate ?? undefined }))
}
...
export async function getById(id: number): Promise<UserSession | null> {
  const db = getDb()
  const [rows] = await db.execute<{ id: number; email: string; role: string; full_name: string; gate: number | null }[]>(
    "SELECT id, email, role, full_name, gate FROM guards WHERE id = ? LIMIT 1",
    [id]
  )
  const row = Array.isArray(rows) ? rows[0] : null
  if (!row) return null
  return { id: row.id, email: row.email, role: row.role as UserRole, fullName: row.full_name, gate: row.gate ?? undefined }
}
