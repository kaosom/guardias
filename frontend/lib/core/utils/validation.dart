...
String formatPueblaPlate(String value) {
  ...
  final numbers = clean.length > 3 ? clean.substring(3, clean.length > 7 ? 7 : clean.length) : '';
...
ValidationResult validatePlateWithMessage(String plate) {
  ...
  if (clean.length < 6) {
    return ValidationResult(isValid: false, error: 'Placa incompleta (mínimo 6 caracteres)');
  }
  
  if (clean.length > 7) {
    return ValidationResult(isValid: false, error: 'Placa demasiado larga (máximo 7 caracteres)');
  }
  ...
  if (!_validFirstLetters.contains(firstLetter)) {
    return ValidationResult(isValid: false, error: 'Primera letra debe ser T o U (Puebla)');
  }
  
  for (int i = 0; i < letters.length; i++) {
    final letter = letters[i];
    if (_excludedLetters.contains(letter)) {
      return ValidationResult(isValid: false, error: 'Letra $letter no permitida (NOM-001-SCT-2-2016)');
    }
  ...
  if (numbers.length < 3 || numbers.length > 4) {
    return ValidationResult(isValid: false, error: 'Se requieren 3 o 4 números');
  }
...
ValidationResult validateMatriculaWithMessage(String matricula) {
  ...
  if (digits.length < 9) {
    return ValidationResult(isValid: false, error: 'Matrícula incompleta (${digits.length}/9 dígitos)');
  }
  
  if (digits.length > 9) {
    return ValidationResult(isValid: false, error: 'Matrícula demasiado larga (máximo 9 dígitos)');
  }
